...
# Importing Data
This pipeline starts by importing data that is outputed by CellRanger (https://support.10xgenomics.com/single-cell-gene-expression/software/pipelines/latest/what-is-cell-ranger). You will get a UMI (unique molecular index) count matrix from CellRanger. 
...
* genes.tsv indicates that he data is from CellRanger v2 and features.tsv indicates that the data is from CellRanger v3 when importing. It's just easier to change the name to features to be consistent with newer pipelines. 
* sometimes it works if you unzip .gz files, but sometimes it doesn't. I would make it a habit to keep it .gz. 
...
* min.cells: genes that are present in at least a n cells
* min.features: cells that are expressed in at least n genes
...
Notes (important slots):

* assays: contains different single-cell assay experimental data (ex: RNA, ATAC, etc.)
    + counts: raw counts
    + data: data worked on (this is be normalized throughout pipeline)
    + scale.data: scaled data
    + var.features: variable features (found through FindVariableFeatures)
...
* It is standard to look at nCount/nUMI, nFeature/nGene, and percentage of mitochondrial reads for each cell. 
* nCount: total # of molecules detected within a cell
* nFeature: total # of genes detected in each cell.

Can also check percentage of ribosomal reads (I won't be using this in this pipeline, but it is an option you can use):
```{r, eval=FALSE}
# human reads start with RPS or RPL
panc8_sub$percent.rb <- PercentageFeatureSet(panc8_sub, pattern = '^RP[SL]')
...
* QC done here places a hard threshold where you set one threshold for all samples
* Can do a more lenient QC by setting a different threshold for each sample. You can see an example in "Pre-Process -> Merge -> Downstream" -> QC Section.
...
* DE is done on the "data" slot. If you have batch corrected values, do not use corrected values for DE. It is better to include batch variables (latent.vars) in the model when running DE. 
